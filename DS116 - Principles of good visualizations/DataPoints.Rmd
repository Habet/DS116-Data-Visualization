...
knitr::opts_chunk$set(echo = F, tidy = F,  collapse = TRUE, warning=F, message = F,
                      size = 'footnotesize')
options(scipen = 999)
library(ggplot2)
library(dplyr)
...
Example of the chart, how can you imporve the data-ink ratio ?
...
ggplot(df, aes(x = City, y = Sales)) + geom_bar(stat='identity') + 
  ...
  labs(title = "Sales by city, '0000 drams")
```

## Chartjunk

What is it that we dont need?
-Redundant axis lables

\scriptsize
```{r, out.height='65%'}
ggplot(df, aes(x = City, y = Sales)) + geom_bar(stat='identity') + 
  geom_label(mapping = aes(label = Sales)) + 
  labs(title = "Sales by city, '0000 drams", x = "", y = "")
```
...
According to Edward Tufte (The visual display of Quantitave Information)
...
"... To be truthful and revealing, data graphics must bear on the question at the heart of quantitative thinking: "Compared to what?" The emaciated, data-thin design should always provoke suspicion, for graphics often lie by omission, leaving out data sufficient for comparision... "

## Principles of good visualization: Principle 6


```{r, echo = F, out.width='95%'}
include_graphics('tufte 7.png')
```
...
```{r, echo=F, out.width='95%'}
include_graphics('tufte 9.png')
```


## Books

```{r, echo = F, fig.align="center", out.height='92%'}
include_graphics('ggplot book.jpg')
```


## Books

```{r, out.height='90%', echo = F, fig.align="center"}
include_graphics('tufte.jpg')
```
...
include_graphics('clevland book.jpg')
```
