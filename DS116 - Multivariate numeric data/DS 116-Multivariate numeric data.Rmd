...
We use geom_tile() to draw rectangles
...
```{r, out.height='65%'}
ggplot(grid_df, aes(x = x, y = y, z = z)) + 
  geom_contour(aes(color = after_stat(level)), binwidth = 2.5) 
```

\tiny
*Note: `after_stat(level)` replaces the deprecated `..level..` syntax (ggplot2 3.4.0+).*
...
\tiny
*Note: `after_stat(density)` replaces the deprecated `..density..` syntax.*
