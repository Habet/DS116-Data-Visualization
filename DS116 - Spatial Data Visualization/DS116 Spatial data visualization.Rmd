...
knitr::opts_chunk$set(echo = TRUE, tidy = F,warning=F, message = F, error = F, size = 'footnotesize')
options(scipen = 999)
library(ggplot2)
library(knitr)
library(dplyr)
...
arm_df = data.frame(Marz = arm_df$Name, st_coordinates(arm_df))
...
\small
```{r}
arm <- arm %>% left_join(gdp, by = c('Name' = "Region"))
arm$Per_Capita_gdp
```
