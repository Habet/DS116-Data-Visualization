...
subtitle: "Intro to R"
author: "Habet Madoyan, Narek Sahakyan"
...
students <- data.frame(
  ...
  year = factor(c("Junior", "Senior", "Sophomore", "Senior", "Junior", "Freshman"),
...
Different ways of assigning values to variables inside functions,
when using <- the value of the argument will be stored inside the variable in the environment.
...
```{r}
outliers <- house_prices[house_prices < lower | house_prices > upper]
...
```{r}
result <- find_outliers(house_prices)
result
```
...
Most summary functions return NA if the data contains missing values. Use the `na.rm = TRUE` argument to exclude NAs.
...
```{r}
teams2 <- rbind(teams, data.frame(team_id = 6, team_name = "Knicks", city = "New York"))
...
```{r, include=FALSE}
task1_solution <- summer %>%
  group_by(Country) %>%
  summarise(
    total_medals = n(),
    unique_sports = n_distinct(Sport)
  ) %>%
...
task2_mean <- mean(usa_yearly$medals)
task2_median <- median(usa_yearly$medals)
population <- data.frame(
  Country = c("USA", "GBR", "GER", "FRA"),
  pop_millions = c(331, 67, 83, 67)
)
medal_counts <- summer %>%
  filter(Country %in% c("USA", "GBR", "GER", "FRA")) %>%
  group_by(Country) %>%
  summarise(total_medals = n())
task3_solution <- medal_counts %>%
  left_join(population, by = "Country") %>%
  mutate(medals_per_million = total_medals / pop_millions)
```
